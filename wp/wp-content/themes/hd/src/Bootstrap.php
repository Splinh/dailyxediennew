...
final class Bootstrap {
	...
	private static function bootCoreServices(): void {
		...
		add_action( 'hd_daily_rate_limit_cleanup', [ RateLimitStorage::class, 'cleanupDb' ] );
	...
	public static function addCronSchedules( array $schedules ): array {
		$schedules['weekly']  ??= [
			...
			'display'  => __( 'Once Weekly', 'hd' ),
		];
		$schedules['monthly'] ??= [
			'interval' => 30 * DAY_IN_SECONDS,
			'display'  => __( 'Once Monthly', 'hd' ),
		];
