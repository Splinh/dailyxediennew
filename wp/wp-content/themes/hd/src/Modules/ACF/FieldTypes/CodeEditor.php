...
class CodeEditor extends \acf_field {
	...
	public function __construct() {
		...
		$this->label    = esc_html__( 'Code Editor', 'hd' );
	...
	public function render_field_settings( $field ) {
		acf_render_field_setting(
			$field,
			[
				'label'        => esc_html__( 'Default Value', 'hd' ),
				'instructions' => esc_html__( 'Appears when creating a new post', 'hd' ),
				'type'         => 'textarea',
				'name'         => 'default_value',
				'rows'         => 4,
			]
		...
		acf_render_field_setting(
			$field,
			[
				'label'        => esc_html__( 'Editor Mode', 'hd' ),
				'instructions' => esc_html__( 'Select the CodeMirror syntax highlighting mode', 'hd' ),
				'type'         => 'select',
				'name'         => 'mode',
				'choices'      => [
					'htmlmixed'  => 'HTML',
		...
		acf_render_field_setting(
			$field,
			[
				'label'        => esc_html__( 'Rows', 'hd' ),
				'instructions' => esc_html__( 'Sets the textarea height (number of visible rows)', 'hd' ),
				'type'         => 'number',
				'name'         => 'rows',
				'min'          => 4,
				'max'          => 40,
		...
		acf_render_field_setting(
			$field,
			[
				'label'        => esc_html__( 'Placeholder Text', 'hd' ),
				'instructions' => esc_html__( 'Appears within the input when empty', 'hd' ),
				'type'         => 'text',
				'name'         => 'placeholder',
			]
		);
