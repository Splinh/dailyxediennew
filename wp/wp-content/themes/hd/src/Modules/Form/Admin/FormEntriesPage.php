...
class FormEntriesPage {
	...
	public static function renderPage(): void {
		...
		echo '<div class="wrap">';
		echo '<h1 class="wp-heading-inline">' . esc_html__( 'Form Entries', 'hd' ) . '</h1>';
		...
		echo '<a href="' . esc_url( $xlsxUrl ) . '" class="page-title-action">' . esc_html__( 'Export XLSX', 'hd' ) . '</a>';
		echo '<a href="' . esc_url( $csvUrl ) . '" class="page-title-action">' . esc_html__( 'Export CSV', 'hd' ) . '</a>';
		...
		$listTable->search_box( __( 'Search', 'hd' ), 'search_id' );
		$listTable->display();
	...
	private static function renderViewPage( int $id ): void {
		...
		if ( ! $entry ) {
			echo '<div class="wrap"><p>' . esc_html__( 'Entry not found.', 'hd' ) . '</p></div>';
			return;
		}
		...
		if ( 'POST' === ( $_SERVER['REQUEST_METHOD'] ?? '' )
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'Sorry, you are not allowed to edit this entry.', 'hd' ) );
			}
			...
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Notes saved.', 'hd' ) . '</p></div>';
		}
		...
		echo '<h1 class="wp-heading-inline">' . esc_html__( 'Entry Details', 'hd' ) . '</h1>';
		echo '<a href="' . esc_url( admin_url( 'admin.php?page=hd-form-entries' ) ) . '" class="page-title-action">' . esc_html__( 'Back', 'hd' ) . '</a>';
		echo '<hr class="wp-header-end">';
		...
		echo '<h2 class="hndle"><span>' . esc_html__( 'Form Data', 'hd' ) . '</span></h2>';
		echo '<div class="inside">';
		echo '<table class="form-table">';

		// Core fields.
		$coreFields = [
			__( 'Name', 'hd' )  => $entry['name'],
			__( 'Email', 'hd' ) => $entry['email'],
			__( 'Phone', 'hd' ) => $entry['phone'],
		];
		...
		if ( ! empty( $files ) ) {
			...
			if ( ! empty( $attachmentLinks ) ) {
				echo '<tr>';
				echo '<th scope="row">' . esc_html__( 'Attachments', 'hd' ) . '</th>';
		...
		echo '<div class="postbox">';
		echo '<h2 class="hndle"><span>' . esc_html__( 'Metadata', 'hd' ) . '</span></h2>';
		echo '<div class="inside">';
		echo '<p><strong>' . esc_html__( 'ID', 'hd' ) . ':</strong> ' . esc_html( $entry['id'] ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Form Type', 'hd' ) . ':</strong> ' . esc_html( $entry['form_type'] ) . '</p>';
		...
		echo '<p><strong>' . esc_html__( 'Status', 'hd' ) . ':</strong> ' . esc_html( $statusLabel ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Date', 'hd' ) . ':</strong> ' . esc_html( $entry['created_at'] ) . '</p>';
		echo '<p><strong>' . esc_html__( 'IP', 'hd' ) . ':</strong> ' . esc_html( $entry['ip_address'] ) . '</p>';
		echo '<hr class="hd-meta-separator" />';
		echo '<p><strong>' . esc_html__( 'Source', 'hd' ) . ':</strong> ' . esc_html( $entry['utm_source'] ?: '-' ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Medium', 'hd' ) . ':</strong> ' . esc_html( $entry['utm_medium'] ?: '-' ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Campaign', 'hd' ) . ':</strong> ' . esc_html( $entry['utm_campaign'] ?: '-' ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Referer', 'hd' ) . ':</strong> ' . ( $entry['referer_url'] ? '<a href="' . esc_url( $entry['referer_url'] ) . '" target="_blank">' . esc_html__( 'Link', 'hd' ) . '</a>' : '-' ) . '</p>';
		echo '</div></div>';
		// ↑ closes .inside + .postbox (Metadata)

		// -- Admin Notes postbox.
		echo '<div class="postbox">';
		echo '<h2 class="hndle"><span>' . esc_html__( 'Admin Notes', 'hd' ) . '</span></h2>';
		echo '<div class="inside">';
		...
		echo '<p><button type="submit" class="button button-primary">' . esc_html__( 'Save Notes', 'hd' ) . '</button></p>';
		echo '</form>';
		echo '</div></div>';
