...
use HD\Modules\Form\Admin\FormEntriesPage;
use HD\Modules\Form\Admin\FormExporter;
use HD\Modules\Form\Admin\FormLogsPage;
use HD\Modules\Form\Admin\FormSettingsPage;
use HD\Modules\Form\API\FormAPI;
use HD\Modules\Form\API\DynamicFieldAPI;
use HD\Core\Helper;
use HD\Modules\Form\Cron\AsyncFormProcessor;
use HD\Modules\Form\Cron\FormEntryCleaner;
use HD\Modules\Form\Cron\MailQueueProcessor;
use HD\Modules\Form\Cron\WeeklyDigestCron;
...
final class FormModule extends AbstractModule implements HasAdminContext, HasDatabaseSchema {
	...
	public function boot(): void {
		...
		// Inject honeypot payload into hdConfig on frontend (after Theme localize at priority 10).
		if ( ! is_admin() ) {
	...
	public static function injectFrontendConfig(): void {
		...
		if ( false !== $json ) {
			wp_add_inline_script(
				'jquery-core',
				sprintf( 'window.hdConfig=window.hdConfig||{};window.hdConfig.form={honeypot:%s};', $json ),
				'before'
			);
		}
	}
