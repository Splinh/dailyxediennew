...
final class OptionsAdmin {
			...
			printf(
				...
				esc_html(
					sprintf(
						...
						__( 'You are editing options for: %s', 'hd' ),
						$currentLang->name
					)
				)
			);
	...
	public function maybeEnqueueAssets(): void {
		...
		wp_localize_script(
			...
			[
				...
				'i18n'    => [
					'translateTo'    => __( 'Translate to %s', 'hd' ),
					'saving'         => __( 'Saving…', 'hd' ),
					'copying'        => __( 'Copying…', 'hd' ),
					'removing'       => __( 'Removing…', 'hd' ),
					'saved'          => __( 'Saved!', 'hd' ),
					'copied'         => __( 'Copied!', 'hd' ),
					'removed'        => __( 'Removed!', 'hd' ),
					'confirmRemove'  => __( 'Remove this translation? Fields will fall back to the default language.', 'hd' ),
					'hasTranslation' => __( 'Has translation', 'hd' ),
					'noTranslation'  => __( 'No translation', 'hd' ),
					'error'          => __( 'An error occurred. Please try again.', 'hd' ),
				],
			]
		);
	}
	...
	public function ajaxRenderForm(): void {
		...
		if ( empty( $fieldGroups ) ) {
			wp_send_json_error( [ 'message' => __( 'No field groups found.', 'hd' ) ] );
		}
	...
	public function ajaxSave(): void {
		...
		if ( acf_validate_save_post( true ) ) {
			...
			wp_send_json_success( [ 'message' => __( 'Translation saved.', 'hd' ) ] );
		}

		wp_send_json_error( [ 'message' => __( 'Validation failed.', 'hd' ) ] );
	}
	...
	public function ajaxCopy(): void {
		...
		if ( $lang === pll_default_language() ) {
			wp_send_json_error( [ 'message' => __( 'Cannot copy to the default language.', 'hd' ) ] );
		}
		...
		if ( empty( $fields ) ) {
			wp_send_json_error( [ 'message' => __( 'No fields to copy.', 'hd' ) ] );
		}
		...
		wp_send_json_success( [ 'message' => __( 'Fields copied.', 'hd' ) ] );
	}
	...
	public function ajaxRemove(): void {
		...
		if ( $lang === pll_default_language() ) {
			wp_send_json_error( [ 'message' => __( 'Cannot remove the default language.', 'hd' ) ] );
		}

		if ( ! PLL()->model->get_language( $lang ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid language.', 'hd' ) ] );
		}

		$page = function_exists( 'acf_get_options_page' ) ? acf_get_options_page( $menuSlug ) : null;
		if ( empty( $page ) || ( $page['post_id'] ?? '' ) !== $postId ) {
			wp_send_json_error( [ 'message' => __( 'Invalid options page.', 'hd' ) ] );
		}
		...
		wp_send_json_success( [ 'message' => __( 'Translation removed.', 'hd' ) ] );
	}
	...
	private function verifyAjaxNonce(): void {
		if ( ! check_ajax_referer( self::NONCE_ACTION, 'nonce', false ) ) {
			wp_send_json_error( [ 'message' => __( 'Security check failed.', 'hd' ) ], 403 );
		}

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( [ 'message' => __( 'Permission denied.', 'hd' ) ], 403 );
		}
	}
	...
	private function getAjaxParams(): array {
		...
		if ( empty( $postId ) || empty( $lang ) || empty( $menuSlug ) ) {
			wp_send_json_error( [ 'message' => __( 'Missing required parameters.', 'hd' ) ] );
		}
