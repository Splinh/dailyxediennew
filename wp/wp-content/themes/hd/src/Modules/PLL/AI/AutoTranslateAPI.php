...
use HD\Modules\PLL\AI\Jobs\BatchManager;
use HD\Modules\PLL\AI\Jobs\JobRepository;
...
final class AutoTranslateAPI extends AbstractAPI {
	...
	public function permission(): bool|\WP_Error {
		return current_user_can( 'manage_options' )
			? true
			: new \WP_Error( 'hd_pll_ai_forbidden', __( 'Insufficient permissions.', 'hd' ), [ 'status' => 403 ] );
	}
	...
	public function createJob( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( ! in_array( $type, [ 'term', 'string' ], true ) ) {
			...
			if ( ! $source instanceof \WP_Post ) {
				return $this->resultResponse( new \WP_Error( 'hd_pll_ai_post_not_found', __( 'Source post not found.', 'hd' ), [ 'status' => 404 ] ) );
			}

			if ( function_exists( 'pll_is_translated_post_type' ) && ! pll_is_translated_post_type( $source->post_type ) ) {
				return $this->resultResponse( new \WP_Error( 'hd_pll_ai_not_translatable', sprintf( __( 'Post type "%s" is not registered for translation in Polylang.', 'hd' ), esc_html( $source->post_type ) ), [ 'status' => 403 ] ) );
			}
		}
	...
	public function runJob( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( ! $job ) {
			return $this->resultResponse( new \WP_Error( 'hd_pll_ai_job_not_found', __( 'Translation job not found.', 'hd' ), [ 'status' => 404 ] ) );
		}
	...
	private function targetLang( \WP_REST_Request $request ): string|\WP_Error {
		...
		if ( ! in_array( $lang, (array) $list, true ) ) {
			return new \WP_Error( 'hd_pll_ai_invalid_target_language', __( 'Invalid target language.', 'hd' ), [ 'status' => 400 ] );
		}
