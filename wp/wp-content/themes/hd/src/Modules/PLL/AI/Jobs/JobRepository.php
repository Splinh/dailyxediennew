...
use HD\Modules\PLL\AI\Enum\JobStatus;
...
final class JobRepository {
	...
	public function update( int $id, array $changes ): bool|\WP_Error {
		$job = $this->get( $id );
		if ( ! $job ) {
			return new \WP_Error( 'hd_pll_ai_job_not_found', __( 'Translation job not found.', 'hd' ) );
		}
	...
	private static function invalidStatusError( mixed $status ): \WP_Error {
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Invalid translation job status: %s', 'hd' ),
