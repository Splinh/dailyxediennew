...
final class TranslationEngine {
	...
	private function translateChunk( array $units, string $sourceLang, string $targetLang ): array|\WP_Error {
		...
		if ( ! empty( $errors ) ) {
			...
			$errors = $this->validator->validate( $units, $translations );
			if ( ! empty( $errors ) ) {
				return new \WP_Error( 'hd_pll_ai_validation_failed', __( 'Translation validation failed.', 'hd' ), [ 'errors' => $errors ] );
			}
		}
	...
	private function parseTranslations( string $content, array $units ): array|\WP_Error {
		...
		if ( ! is_array( $decoded ) ) {
			return new \WP_Error( 'hd_pll_ai_json_parse_failed', __( 'AI response is not valid JSON.', 'hd' ) );
		}

		$items = $decoded['translations'] ?? $decoded;
		if ( ! is_array( $items ) ) {
			return new \WP_Error( 'hd_pll_ai_missing_translations', __( 'AI response is missing translations.', 'hd' ) );
		}
		...
		if ( ! empty( $missingIds ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'AI response is missing translations for: %s', 'hd' ),
					implode( ', ', $missingIds )
				)
			);
		}
