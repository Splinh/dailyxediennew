...
final class LanguageField {
		...
		foreach ( $this->getTranslatablePostTypes() as $post_type ) {
			\register_rest_field(
				...
				[
					...
					'schema'          => [
						'description' => __( 'Polylang language slug.', 'hd' ),
						'type'        => 'string',
						'context'     => [ 'view', 'edit' ],
					],
				]
			...
			\register_rest_field(
				...
				[
					...
					'schema'          => [
						'description'          => __( 'Map of Polylang language slug to translated post ID.', 'hd' ),
						'type'                 => 'object',
		...
		foreach ( $this->getTranslatableTaxonomies() as $taxonomy ) {
			...
			\register_rest_field(
				...
				[
					...
					'schema'          => [
						'description'          => __( 'Map of Polylang language slug to translated term ID.', 'hd' ),
						'type'                 => 'object',
						'additionalProperties' => [ 'type' => 'integer' ],
						'context'              => [ 'view', 'edit' ],
					],
	...
	public function updatePostTranslations( mixed $value, \WP_Post $post ): bool|\WP_Error {
		if ( ! is_array( $value ) ) {
			return new \WP_Error(
				...
				__( 'The translations field must be an object (lang_slug => post_id).', 'hd' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function updateTermTranslations( mixed $value, \WP_Term $term ): bool|\WP_Error {
		if ( ! is_array( $value ) ) {
			return new \WP_Error(
				'pll_rest_invalid_translations',
				__( 'The translations field must be an object (lang_slug => term_id).', 'hd' ),
				[ 'status' => 400 ]
			);
		}
