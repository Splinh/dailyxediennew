...
use HD\Core\Helper;
use HD\Modules\PLL\AI\AiClient;
use HD\Modules\PLL\PLLModule;
use HD\Modules\PLL\ImportExport\ExportHandler;
use HD\Modules\PLL\ImportExport\FileFormatFactory;
use HD\Modules\PLL\ImportExport\ImportHandler;
use HD\Modules\PLL\Translation\Scanner;
use HD\Modules\PLL\Translation\Settings as TranslationSettings;
...
final class PLLSettings {
	...
	public static function addTab( array $tabs ): array {
		$tabs[ self::TAB_SLUG ] = __( 'HD Polylang', 'hd' );
	...
	public static function renderTab(): void {
		...
		if ( $msg ) {
			$messages = [
				'saved'        => __( 'Settings saved.', 'hd' ),
				'imported'     => sprintf(
					/* translators: %d: number of imported items */
					__( 'Translations imported: %d items.', 'hd' ),
		...
		<div class="form-wrap">
			...
			<div class="notice notice-success inline" style="margin: 0 0 20px 0;">
				<p><span class="dashicons dashicons-yes-alt" style="color: #00a32a;"></span> <strong><?php esc_html_e( 'WooCommerce Integration Active', 'hd' ); ?>:</strong> <?php esc_html_e( 'Native translation support for products and emails is running automatically.', 'hd' ); ?></p>
			</div>
			<?php endif; ?>
			...
			<form method="post" enctype="multipart/form-data" action="">
				...
				<?php if ( $show_pro ) : ?>
				<!-- Pro Features -->
				<h3><?php esc_html_e( 'Pro Features', 'hd' ); ?></h3>
				<p class="description"><?php esc_html_e( 'These features replace Polylang Pro. Disable this section by activating the Polylang Pro plugin.', 'hd' ); ?></p>
				...
				<h3><?php esc_html_e( 'Admin Dashboard Language', 'hd' ); ?></h3>
				<p class="description"><?php esc_html_e( 'Control the language used for the WordPress admin dashboard.', 'hd' ); ?></p>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Admin Language', 'hd' ); ?></th>
						<td>
							<?php $force_locale = $pll_settings['admin_force_locale'] ?? 'content'; ?>
							<select name="hd_pll_admin_force_locale">
								<option value="content" <?php selected( $force_locale, 'content' ); ?>><?php esc_html_e( 'Content language (default)', 'hd' ); ?></option>
								<option value="default" <?php selected( $force_locale, 'default' ); ?>><?php esc_html_e( 'Always use default language', 'hd' ); ?></option>
								<option value="profile" <?php selected( $force_locale, 'profile' ); ?>><?php esc_html_e( 'Use user profile language', 'hd' ); ?></option>
							</select>
							<p class="description"><?php esc_html_e( 'Choose how the admin dashboard language is determined. "Content language" follows the content you are editing.', 'hd' ); ?></p>
						</td>
					</tr>
				</table>
				...
				<h3><?php esc_html_e( 'Theme & Plugin Translation', 'hd' ); ?></h3>
				<p class="description"><?php esc_html_e( 'Select themes and plugins to scan for translatable strings. Strings will appear in Languages > String translations.', 'hd' ); ?></p>

				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Themes', 'hd' ); ?></th>
						<td>
							<fieldset>
					...
					<tr>
						<th scope="row"><?php esc_html_e( 'Plugins', 'hd' ); ?></th>
						<td>
							<fieldset>
				...
				<h3><?php esc_html_e( 'Translation Import/Export', 'hd' ); ?></h3>
				<p class="description"><?php esc_html_e( 'Export string translations in CSV, PO, or XLIFF format. Import translated files back.', 'hd' ); ?></p>
					<?php
				...
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Export Strings', 'hd' ); ?></th>
						<td>
							<fieldset>
								<legend class="screen-reader-text"><?php esc_html_e( 'Target languages', 'hd' ); ?></legend>
								<p><strong><?php esc_html_e( 'Target languages:', 'hd' ); ?></strong></p>
							...
							<p style="margin-top:8px;">
								<label for="hd-pll-export-group"><?php esc_html_e( 'Filter group:', 'hd' ); ?></label>
								<select name="hd_pll_export_group" id="hd-pll-export-group">
									<option value=""><?php esc_html_e( 'All groups', 'hd' ); ?></option>
							...
							<p style="margin-top:8px;">
								<strong><?php esc_html_e( 'File format:', 'hd' ); ?></strong><br>
							...
							<p>
								<button type="submit" name="hd_pll_export" value="1" class="button button-secondary">
									<?php esc_html_e( 'Download', 'hd' ); ?>
								</button>
							</p>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Import Translations', 'hd' ); ?></th>
						<td>
							<input type="file" name="hd_pll_import_file" accept=".csv,.po,.xliff,.xlf">
							<p class="description"><?php esc_html_e( 'Upload a CSV, PO, or XLIFF file to import string translations.', 'hd' ); ?></p>
						</td>
					</tr>
				</table>
				<?php endif; ?>
	...
	private static function renderAiSettings( array $settings ): void {
		...
		<h3>
			<?php esc_html_e( 'AI Translation', 'hd' ); ?>
			<b style="font-size:14px;color:<?php echo AiClient::isAvailable() ? '#00a32a' : '#d63638'; ?>">
				<?php echo AiClient::isAvailable() ? esc_html__( 'HDAT route available', 'hd' ) : esc_html__( 'HDAT route missing', 'hd' ); ?>
			</b>
		</h3>
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Enable AI translation', 'hd' ); ?></th>
				<td>
					<label>
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'HDAT consumer token', 'hd' ); ?></th>
				<td>
					...
					<p class="description"><?php esc_html_e( 'Model and provider routing are managed centrally by HDAT Credentials associated with this token.', 'hd' ); ?></p>
					<?php
					if ( $token ) {
						...
						if ( null !== $auth ) {
							if ( is_wp_error( $auth ) ) {
								...
								printf( '<p class="description" style="color: #d63638;"><strong>%s</strong></p>', esc_html__( 'Token not found or invalid.', 'hd' ) );
							} else {
								printf( '<p class="description" style="color: #00a32a;"><strong>✓ %s</strong></p>', esc_html__( 'Active', 'hd' ) );
							}
						}
					}
					?>
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'Target languages', 'hd' ); ?></th>
				<td>
					<fieldset>
						<legend class="screen-reader-text"><span><?php esc_html_e( 'Target languages', 'hd' ); ?></span></legend>
					...
					<p class="description"><?php esc_html_e( 'Select the languages to generate translations for when performing bulk actions.', 'hd' ); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Bulk draft status', 'hd' ); ?></th>
				<td>
					<select name="hd_pll_ai_post_status">
						<option value="draft" <?php selected( $settings['ai_default_post_status'] ?? 'draft', 'draft' ); ?>><?php esc_html_e( 'Draft', 'hd' ); ?></option>
						<option value="pending" <?php selected( $settings['ai_default_post_status'] ?? 'draft', 'pending' ); ?>><?php esc_html_e( 'Pending review', 'hd' ); ?></option>
					</select>
					<p class="description"><?php esc_html_e( 'Bulk AI translation always creates translated posts in the selected status. Editor assist remains preview-only inside the editor.', 'hd' ); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Content types', 'hd' ); ?></th>
				<td>
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'Fields', 'hd' ); ?></th>
				<td>
					<label><input type="checkbox" name="hd_pll_ai_translate_title" value="1" <?php checked( ! empty( $settings['ai_translate_title'] ) ); ?>> <?php esc_html_e( 'Title', 'hd' ); ?></label>
					<label><input type="checkbox" name="hd_pll_ai_translate_content" value="1" <?php checked( ! empty( $settings['ai_translate_content'] ) ); ?>> <?php esc_html_e( 'Content', 'hd' ); ?></label>
					<label><input type="checkbox" name="hd_pll_ai_translate_excerpt" value="1" <?php checked( ! empty( $settings['ai_translate_excerpt'] ) ); ?>> <?php esc_html_e( 'Excerpt', 'hd' ); ?></label>
					<label><input type="checkbox" name="hd_pll_ai_translate_slug" value="1" <?php checked( ! empty( $settings['ai_translate_slug'] ) ); ?>> <?php esc_html_e( 'Slug', 'hd' ); ?></label>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Meta keys', 'hd' ); ?></th>
				<td>
					...
					<p class="description"><?php esc_html_e( 'One key per line. List of custom field keys to translate (e.g., _yoast_wpseo_title, rank_math_description). Do not add fields storing IDs, media, or arrays.', 'hd' ); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Limits', 'hd' ); ?></th>
				<td>
					<fieldset>
						<legend class="screen-reader-text"><span><?php esc_html_e( 'Limits', 'hd' ); ?></span></legend>
						<label style="display: block; margin-bottom: 8px;">
							...
							<?php esc_html_e( 'Maximum translation units per API request', 'hd' ); ?>
						</label>
						<label style="display: block; margin-bottom: 8px;">
							...
							<?php esc_html_e( 'Maximum characters per API request', 'hd' ); ?>
						</label>
						<p class="description"><?php esc_html_e( 'Controls chunking. Long posts will be split into smaller API requests to prevent timeouts or token limits.', 'hd' ); ?></p>
					</fieldset>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Glossary', 'hd' ); ?></th>
				<td>
					...
					<p class="description"><?php esc_html_e( 'One term per line. AI will be forced to preserve these terms (e.g. brand names, SKUs) without translating them.', 'hd' ); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Editor assist', 'hd' ); ?></th>
				<td>
					<label><input type="checkbox" name="hd_pll_ai_editor_assist" value="1" <?php checked( ! empty( $settings['ai_editor_assist_enabled'] ) ); ?>> <?php esc_html_e( 'Enable editor-time assist', 'hd' ); ?></label>
					<p class="description"><?php esc_html_e( 'When creating a new translation in the Editor, automatically run AI in the background to pre-fill the title and content. You can review before publishing.', 'hd' ); ?></p>
				</td>
			</tr>
		</table>
		<?php
	...
	private static function getProFeatureLabels(): array {
		return [
			'translate_slugs'   => __( 'Translate URL Slugs', 'hd' ),
			'duplicate_content' => __( 'Duplicate Content on Translation', 'hd' ),
			'share_slugs'       => __( 'Share Slugs Across Languages', 'hd' ),
			'locale_fallback'   => __( 'Locale Fallback', 'hd' ),
		];
	}
