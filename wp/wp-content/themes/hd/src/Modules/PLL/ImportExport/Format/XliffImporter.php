...
final class XliffImporter implements ImporterInterface {
	...
	public function importFromFile( string $filepath ): bool|\WP_Error {
		if ( ! extension_loaded( 'libxml' ) ) {
			return new \WP_Error( 'pll_xliff_no_libxml', __( 'Error: PHP libxml extension is required.', 'hd' ) );
		}
		...
		if ( false === $content || '' === $content ) {
			...
			return new \WP_Error( 'pll_xliff_read_error', __( 'Error: Could not read XLIFF file.', 'hd' ) );
		}
		...
		if ( ! $dom->loadXML( $content, LIBXML_NONET ) ) {
			...
			return new \WP_Error( 'pll_xliff_parse_error', __( 'Error: Invalid XML in XLIFF file.', 'hd' ) );
		}
	...
	private function parseDocument( \DOMDocument $dom ): bool|\WP_Error {
		...
		if ( null === $xliff || 'xliff' !== $xliff->localName ) {
			return new \WP_Error( 'pll_xliff_invalid', __( 'Error: Not a valid XLIFF document.', 'hd' ) );
		}
		...
		if ( empty( $this->entries ) ) {
			return new \WP_Error( 'pll_xliff_empty', __( 'Error: No translation units found in XLIFF file.', 'hd' ) );
		}
