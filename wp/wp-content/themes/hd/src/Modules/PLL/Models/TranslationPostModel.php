 ...
 *
 * @package HD\Modules\PLL\Models
 */

namespace HD\Modules\PLL\Models;

use HD\Core\DB;
use HD\Modules\PLL\ACF\Entity\PostEntity;
use HD\Modules\PLL\AI\Translator\MetaTranslator;
use HD\Modules\PLL\PLLModule;
use HD\Modules\PLL\Pro\SyncContent;
...
final class TranslationPostModel {
	...
	public function duplicate( int $sourceId, string $targetLang, array $translatedFields = [], array $options = [] ): int|\WP_Error {
		...
		if ( ! $source instanceof \WP_Post ) {
			return new \WP_Error( 'hd_pll_source_not_found', __( 'Source post not found.', 'hd' ) );
		}

		$sourceLang = \pll_get_post_language( $sourceId );
		if ( ! $sourceLang || ! \PLL()->model->get_language( $targetLang ) ) {
			return new \WP_Error( 'hd_pll_invalid_language', __( 'Invalid source or target language.', 'hd' ) );
		}
		...
		if ( $targetId > 0 ) {
			...
			if ( ! $target instanceof \WP_Post || $target->post_type !== $source->post_type ) {
				return new \WP_Error( 'hd_pll_invalid_target', __( 'Invalid target post.', 'hd' ) );
			}

			$targetLangCurrent = \pll_get_post_language( $targetId );
			if ( $targetLangCurrent && $targetLangCurrent !== $targetLang ) {
				return new \WP_Error( 'hd_pll_target_language_mismatch', __( 'Target post language does not match requested language.', 'hd' ) );
			}

			$linkedTarget = \pll_get_post( $sourceId, $targetLang );
			if ( $linkedTarget && (int) $linkedTarget !== $targetId ) {
				return new \WP_Error( 'hd_pll_target_conflict', __( 'A different translation already exists for this language.', 'hd' ) );
			}
		} else {
		...
		if ( $targetId > 0 && ! $overwrite ) {
			return new \WP_Error( 'hd_pll_target_exists', __( 'Translation already exists. Use overwrite option.', 'hd' ) );
		}
