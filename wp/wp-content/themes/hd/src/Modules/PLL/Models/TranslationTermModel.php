...
final class TranslationTermModel {
	...
	public function duplicate( int $sourceTermId, string $targetLang, array $translatedFields = [], array $options = [] ): int|\WP_Error {
		...
		if ( ! $term instanceof \WP_Term ) {
			return new \WP_Error( 'hd_pll_source_term_not_found', __( 'Source term not found.', 'hd' ) );
		}
		...
		if ( $targetId > 0 && ! $overwrite ) {
			return new \WP_Error( 'hd_pll_target_term_exists', __( 'Term translation already exists. Use overwrite option.', 'hd' ) );
		}
		...
		if ( $term->parent ) {
			...
			if ( ! $parent ) {
				return new \WP_Error( 'hd_pll_missing_parent_term', __( 'Translated parent term is missing.', 'hd' ) );
			}
			$args['parent'] = $parent;
		}
