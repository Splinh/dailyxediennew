 ...
 *
 * @package HD\Modules\WooCommerce\Filter\API
 */

namespace HD\Modules\WooCommerce\Filter\API;

use HD\API\AbstractAPI;
use HD\Modules\WooCommerce\Filter\FilterManager;
use HD\Modules\WooCommerce\Filter\FilterRegistry;
use HD\Modules\WooCommerce\Filter\Frontend\FilterRenderer;
...
final class FilterAPI extends AbstractAPI {
	...
	private function terms( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( ! taxonomy_exists( $taxonomy ) ) {
			return $this->sendResponse( [ 'message' => __( 'Invalid taxonomy.', 'hd' ) ], 400 );
		}
