 ...
 *
 * @package HD\Modules\WooCommerce\Filter\Admin
 */

namespace HD\Modules\WooCommerce\Filter\Admin;

use HD\Modules\WooCommerce\Filter\Enum\AdoptiveMode;
use HD\Modules\WooCommerce\Filter\FilterMeta;
use HD\Modules\WooCommerce\Filter\FilterRegistry;
...
final class FilterPresetEditor {
	...
	public function renderConfigBox( \WP_Post $post ): void {
		...
		<div class="hd-preset-editor">
			...
			<table class="form-table">
				<tr>
					<th scope="row"><label for="hd_preset_layout"><?php esc_html_e( 'Default Layout', 'hd' ); ?></label></th>
					<td>
						<select id="hd_preset_layout" name="hd_preset_layout">
							<option value="vertical" <?php selected( $layout, 'vertical' ); ?>><?php esc_html_e( 'Vertical (Sidebar Accordion)', 'hd' ); ?></option>
							<option value="horizontal" <?php selected( $layout, 'horizontal' ); ?>><?php esc_html_e( 'Horizontal (Top Bar Popover)', 'hd' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="hd_preset_trigger"><?php esc_html_e( 'Trigger Mode', 'hd' ); ?></label></th>
					<td>
						<select id="hd_preset_trigger" name="hd_preset_trigger">
							<option value="auto" <?php selected( $trigger, 'auto' ); ?>><?php esc_html_e( 'Auto (instant AJAX)', 'hd' ); ?></option>
							<option value="manual" <?php selected( $trigger, 'manual' ); ?>><?php esc_html_e( 'Manual (Apply button)', 'hd' ); ?></option>
							<option value="hybrid" <?php selected( $trigger, 'hybrid' ); ?>><?php esc_html_e( 'Hybrid (auto desktop, manual mobile)', 'hd' ); ?></option>
						</select>
					</td>
				</tr>
			</table>
			...
			<?php // ── Repeater ── ?>
			<h3><?php esc_html_e( 'Filter Items', 'hd' ); ?></h3>
			<p class="description"><?php esc_html_e( 'Add, configure, and reorder filter items. Drag to reorder.', 'hd' ); ?></p>
	...
	private function renderRow( int|string $index, array $item, array $filterTypes, array $productTaxonomies, array $productAttributes ): void {
		...
		<div class="hd-filter-row" data-index="<?php echo esc_attr( (string) $index ); ?>">
			<div class="hd-filter-row__header">
				<span class="hd-filter-row__drag dashicons dashicons-move" title="<?php esc_attr_e( 'Drag to reorder', 'hd' ); ?>"></span>
				<strong class="hd-filter-row__title"><?php echo esc_html( $item['label'] ?: __( 'New Filter', 'hd' ) ); ?></strong>
				...
				<label class="hd-filter-row__enabled">
					...
					<?php esc_html_e( 'Enabled', 'hd' ); ?>
				</label>
				<button type="button" class="hd-filter-row__toggle dashicons dashicons-arrow-down-alt2" title="<?php esc_attr_e( 'Toggle', 'hd' ); ?>"></button>
				<button type="button" class="hd-filter-row__remove dashicons dashicons-trash" title="<?php esc_attr_e( 'Remove', 'hd' ); ?>"></button>
			</div>
			...
			<div class="hd-filter-row__body" style="display:none;">
				<table class="form-table">
					<?php // ── Core Fields ── ?>
					<tr>
						<th><label><?php esc_html_e( 'Type', 'hd' ); ?></label></th>
						<td>
					...
					<tr>
						...
						<td>
							...
							<p class="description"><?php esc_html_e( 'Identifier for URL parameters (e.g., product_cat, pa_color). Auto-generated from taxonomy or type if empty.', 'hd' ); ?></p>
						</td>
					</tr>

					<?php // ── Taxonomy Selector (non-attribute) ── ?>
					<tr class="hd-cond-taxonomy-only">
						<th><label><?php esc_html_e( 'Taxonomy', 'hd' ); ?></label></th>
						<td>
							<select name="<?php echo esc_attr( $prefix ); ?>[taxonomy]" class="hd-taxonomy-select">
								<option value=""><?php esc_html_e( '— Select —', 'hd' ); ?></option>
								<?php
								// Group by type: categories, tags, custom.
								$groups     = [
									__( 'Categories', 'hd' ) => [ 'product_cat' ],
									__( 'Tags', 'hd' ) => [ 'product_tag' ],
								];
								$usedKeys   = [ 'product_cat', 'product_tag' ];
								$customKeys = array_keys( array_diff_key( $productTaxonomies, array_flip( $usedKeys ) ) );
								if ( $customKeys ) {
									$groups[ __( 'Custom', 'hd' ) ] = $customKeys;
								}
					...
					<tr class="hd-cond-attribute-only">
						<th><label><?php esc_html_e( 'Attribute', 'hd' ); ?></label></th>
						<td>
					...
					<tr class="hd-cond-display">
						<th><label><?php esc_html_e( 'Display', 'hd' ); ?></label></th>
						<td>
					...
					<?php // ── Adoptive ── ?>
					<tr>
						<th><label><?php esc_html_e( 'Zero-Count Behavior', 'hd' ); ?></label></th>
						<td>
							<select name="<?php echo esc_attr( $prefix ); ?>[adoptive]">
								<option value="show" <?php selected( $item['adoptive'], 'show' ); ?>><?php esc_html_e( 'Show', 'hd' ); ?></option>
								<option value="hide" <?php selected( $item['adoptive'], 'hide' ); ?>><?php esc_html_e( 'Hide', 'hd' ); ?></option>
								<option value="disable" <?php selected( $item['adoptive'], 'disable' ); ?>><?php esc_html_e( 'Disable', 'hd' ); ?></option>
							</select>
						</td>
					</tr>
					...
					<tr class="hd-cond-has-taxonomy">
						<th><label><?php esc_html_e( 'Order By', 'hd' ); ?></label></th>
						<td>
							<select name="<?php echo esc_attr( $prefix ); ?>[orderby]">
								<option value="name_asc" <?php selected( $item['orderby'], 'name_asc' ); ?>><?php esc_html_e( 'Name A→Z', 'hd' ); ?></option>
								<option value="name_desc" <?php selected( $item['orderby'], 'name_desc' ); ?>><?php esc_html_e( 'Name Z→A', 'hd' ); ?></option>
								<option value="count_desc" <?php selected( $item['orderby'], 'count_desc' ); ?>><?php esc_html_e( 'Count (most first)', 'hd' ); ?></option>
								<option value="menu_order" <?php selected( $item['orderby'], 'menu_order' ); ?>><?php esc_html_e( 'Menu Order', 'hd' ); ?></option>
							</select>
						</td>
					</tr>
					...
					<?php // ── UX Options ── ?>
					<tr>
						<th><?php esc_html_e( 'Options', 'hd' ); ?></th>
						<td>
							<label><input type="checkbox" name="<?php echo esc_attr( $prefix ); ?>[more_less]" value="1" <?php checked( $item['more_less'] ); ?>> <?php esc_html_e( 'Show More/Less toggle', 'hd' ); ?></label>
							...
							<br>
							<label><input type="checkbox" name="<?php echo esc_attr( $prefix ); ?>[searchable]" value="1" <?php checked( $item['searchable'] ); ?>> <?php esc_html_e( 'Searchable (inline term search)', 'hd' ); ?></label>
							<br>
							<label><input type="checkbox" name="<?php echo esc_attr( $prefix ); ?>[show_chips]" value="1" <?php checked( $item['show_chips'] ); ?>> <?php esc_html_e( 'Show in active filter chips', 'hd' ); ?></label>
							<br>
							<label><input type="checkbox" name="<?php echo esc_attr( $prefix ); ?>[collapse]" value="1" <?php checked( $item['collapse'] ); ?>> <?php esc_html_e( 'Collapsed by default (vertical)', 'hd' ); ?></label>
						</td>
					</tr>

					<?php // ── Exclude/Include Terms ── ?>
					<tr class="hd-cond-has-taxonomy">
						<th><label><?php esc_html_e( 'Exclude Terms', 'hd' ); ?></label></th>
						<td>
							<?php
							...
							<select name="<?php echo esc_attr( $prefix ); ?>[exclude_terms][]" multiple
								...
								data-placeholder="<?php esc_attr_e( 'Select terms to exclude…', 'hd' ); ?>">
								<?php
							...
							<br>
							<label><input type="checkbox" name="<?php echo esc_attr( $prefix ); ?>[include_mode]" value="1" <?php checked( $item['include_mode'] ?? false ); ?>> <?php esc_html_e( 'Invert: use as whitelist (include only these terms)', 'hd' ); ?></label>
						</td>
					</tr>

					<?php // ── Price Range Config ── ?>
					<tr class="hd-cond-price">
						<th><label><?php esc_html_e( 'Price Mode', 'hd' ); ?></label></th>
						<td>
							<select name="<?php echo esc_attr( $prefix ); ?>[mode]">
								<option value="custom_ranges" <?php selected( $item['mode'], 'custom_ranges' ); ?>><?php esc_html_e( 'Custom Ranges', 'hd' ); ?></option>
								<option value="slider" <?php selected( $item['mode'], 'slider' ); ?>><?php esc_html_e( 'Slider', 'hd' ); ?></option>
							</select>
						</td>
					</tr>
					<tr class="hd-cond-slider">
						<th><label><?php esc_html_e( 'Slider Config', 'hd' ); ?></label></th>
						<td>
							<div style="display: flex; gap: 15px; align-items: center;">
								<label><?php esc_html_e( 'Min Price', 'hd' ); ?><br>
									<input type="number" name="<?php echo esc_attr( $prefix ); ?>[min]" value="<?php echo absint( $item['min'] ?? 0 ); ?>" min="0" step="10000" style="width: 120px;">
								</label>
								<label><?php esc_html_e( 'Max Price', 'hd' ); ?><br>
								...
								<label><?php esc_html_e( 'Step', 'hd' ); ?><br>
							...
							<p class="description"><?php esc_html_e( 'Define the fixed price bounds for the slider (e.g., 0 to 10000000).', 'hd' ); ?></p>
						</td>
					</tr>
					<tr class="hd-cond-price">
						<th><label><?php esc_html_e( 'Price Ranges', 'hd' ); ?></label></th>
						<td>
							...
							<button type="button" class="button button-small hd-price-range-add">
								<?php esc_html_e( '+ Thêm khoảng giá', 'hd' ); ?>
							</button>
						</td>
					</tr>
				</table>
	...
	private static function renderPriceRow( string $prefix, int|string $ri, array $range ): void {
		...
		<div class="hd-price-range-row">
			<input type="text" name="<?php echo $rPrefix; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>[label]" value="<?php echo esc_attr( $range['label'] ?? '' ); ?>" placeholder="<?php esc_attr_e( 'Tên hiển thị', 'hd' ); ?>" class="hd-price-range-label">
			<input type="number" name="<?php echo $rPrefix; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>[min]" value="<?php echo esc_attr( $min ); ?>" placeholder="<?php esc_attr_e( 'Giá từ', 'hd' ); ?>" min="0" step="1000" class="hd-price-range-min">
			<span class="hd-price-range-sep">—</span>
			<input type="number" name="<?php echo $rPrefix; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>[max]" value="<?php echo esc_attr( $max ); ?>" placeholder="<?php esc_attr_e( 'Giá đến', 'hd' ); ?>" min="0" step="1000" class="hd-price-range-max">
			<button type="button" class="hd-price-range-remove dashicons dashicons-no-alt" title="<?php esc_attr_e( 'Xoá', 'hd' ); ?>"></button>
		</div>
		<?php
	}
