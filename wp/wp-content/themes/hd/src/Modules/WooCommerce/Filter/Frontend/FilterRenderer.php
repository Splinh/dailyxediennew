 ...
 *
 * @package HD\Modules\WooCommerce\Filter\Frontend
 */

namespace HD\Modules\WooCommerce\Filter\Frontend;

use HD\Modules\WooCommerce\Filter\FilterManager;
...
final class FilterRenderer {
		...
		foreach ( $configs as $config ) {
			...
			foreach ( $values as $value ) {
				...
				$chips[]   = sprintf(
					...
					esc_attr( sprintf( __( 'Remove %s', 'hd' ), $label ) )
				);
			}
		}
	...
	private static function resolveValueLabel( array $config, string $value ): string {
		...
		// Rating
		if ( 'rating' === $type ) {
			return sprintf( __( '%s stars', 'hd' ), $value );
		}
