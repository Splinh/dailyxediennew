 ...
 *
 * @package HD\Modules\WooCommerce\Gallery
 */

namespace HD\Modules\WooCommerce\Gallery;
	...
	public static function settingsFields(): array {
		return [
			'gallery_layout'            => [
				'type'    => 'select',
				'options' => [
					'below'   => __( 'Slider — Thumbs Below', 'hd' ),
					'above'   => __( 'Slider — Thumbs Above', 'hd' ),
					'left'    => __( 'Slider — Thumbs Left', 'hd' ),
					'right'   => __( 'Slider — Thumbs Right', 'hd' ),
					'stacked' => __( 'Stacked (Grid, no slider)', 'hd' ),
				],
			],
			'gallery_zoom'              => [
				'type'    => 'toggle',
			...
			'gallery_zoom_scale'        => [
				...
				'max'     => 5,
				'step'    => 0.5,
				'help'    => __( 'Zoom magnification level', 'hd' ),
			],
			'gallery_lens_size'         => [
				'type'    => 'number',
				'default' => 150,
				...
				'max'     => 400,
				'step'    => 10,
				'help'    => __( 'Lens diameter in px (circle mode only)', 'hd' ),
			],
			'gallery_lens_mode'         => [
				'type'    => 'select',
				'options' => [
					'circle' => __( 'Circle (magnifying glass)', 'hd' ),
					'full'   => __( 'Full (lens fills entire image)', 'hd' ),
				],
			],
			'gallery_variation_mode'    => [
				'type'    => 'select',
				'options' => [
					'replace' => __( 'Replace — show only variation images', 'hd' ),
					'prepend' => __( 'Prepend — variation images first, then product gallery', 'hd' ),
				],
			],
			...
			'gallery_product_video_pos' => [
				'type'    => 'select',
				'options' => [
					'first_slide' => __( 'First Slide', 'hd' ),
					'last_slide'  => __( 'Last Slide', 'hd' ),
					'overlay'     => __( 'Floating Overlay Button', 'hd' ),
				],
			],
			'gallery_object_fit'        => [
				'type'    => 'select',
				'options' => [
					'contain' => __( 'Contain (keep full image, may show background)', 'hd' ),
					'cover'   => __( 'Cover (fill frame, may crop edges)', 'hd' ),
				],
			],
			'gallery_thumbs_mobile'     => [
				...
				'min'     => 0,
				'max'     => 6,
				'help'    => __( '0 = auto (CSS-based sizing)', 'hd' ),
			],
			...
			'gallery_nav_arrows'        => [
				'type'    => 'toggle',
				'default' => true,
				'help'    => __( 'Show prev/next navigation arrows on slider', 'hd' ),
			],
		];
	}
