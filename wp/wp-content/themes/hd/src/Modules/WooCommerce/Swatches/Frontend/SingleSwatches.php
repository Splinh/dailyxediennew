 ...
 *
 * @package HD\Modules\WooCommerce\Swatches\Frontend
 */

namespace HD\Modules\WooCommerce\Swatches\Frontend;

use HD\Core\Helper;
use HD\Modules\WooCommerce\Swatches\Admin\ProductSwatchesTab;
use HD\Modules\WooCommerce\Swatches\SwatchConfig;
use HD\Modules\WooCommerce\Swatches\SwatchMeta;
use HD\Modules\WooCommerce\Swatches\SwatchRenderer;
use WC_Product;
		...
		foreach ( $product->get_children() as $variationId ) {
			...
			if ( 0 === $threshold || $qty <= $threshold ) {
				$map[ $value ] = [
					...
					'text'   => sprintf( _n( '%d left', '%d left', $qty, 'hd' ), $qty ),
