...
trait WpTemplate {
	...
	public static function breadCrumbs(): void {
		...
		// Home
		$breadcrumbs[] = '<li><a class="home" href="' . esc_url( self::home() ) . '">' . esc_html__( 'Trang chủ', 'hd' ) . '</a></li>';
		...
		if ( function_exists( 'is_shop' ) && \is_shop() && self::isWoocommerceActive() ) {
			...
			if ( $postType ) {
				...
				if ( ! $taxonomies ) {
					...
					$term = \HD_Query::primaryTerm( $post );
					if ( $term ) {
			...
			$breadcrumbs[] = $before . sprintf( esc_html__( 'Kết quả tìm kiếm cho: %s', 'hd' ), get_search_query() ) . $after;
		} elseif ( is_tag() ) {
			$breadcrumbs[] = $before . sprintf( esc_html__( 'Lưu trữ: %s', 'hd' ), esc_html( single_tag_title( '', false ) ) ) . $after;
		} elseif ( is_author() ) {
			global $author;
			...
			$breadcrumbs[] = $before . esc_html__( 'Không tìm thấy', 'hd' ) . $after;
		}

		// Pagination
		if ( get_query_var( 'paged' ) && (int) get_query_var( 'paged' ) > 1 ) {
			$breadcrumbs[] = $before . ' (' . esc_html__( 'trang', 'hd' ) . ' ' . absint( get_query_var( 'paged' ) ) . ')' . $after;
		}
