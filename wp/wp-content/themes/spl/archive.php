<?php
/**
 * The template for displaying archive posts (category, tag, date, author, search).
 *
...
if ( $is_search ) {
	$archive_title = sprintf( __( 'Kết quả tìm kiếm: "%s"', 'spl' ), get_search_query() );
	$archive_desc  = sprintf( __( 'Tìm thấy %d kết quả', 'spl' ), $wp_query->found_posts );
} elseif ( $is_cat ) {
	$archive_title = single_cat_title( '', false );
	$archive_desc  = category_description( $object->term_id );
} elseif ( $is_tag ) {
	$archive_title = single_tag_title( '', false );
	$archive_desc  = tag_description( $object->term_id );
} elseif ( is_author() ) {
...
if ( $all_cats && ! is_wp_error( $all_cats ) ) :
	$current_cat_id = $is_cat ? $object->term_id : 0;
	...
	<div class="max-w-7xl mx-auto px-4 pt-8 md:pt-10">
		<div class="flex overflow-x-auto gap-2 pb-1" style="-ms-overflow-style:none;scrollbar-width:none;">
			<a href="<?php echo esc_url( $blog_url ); ?>"
			   class="px-5 py-2.5 text-xs font-bold rounded-full transition-all whitespace-nowrap <?php echo ! $is_cat ? 'bg-[#1e73be] text-white shadow-md shadow-[#1e73be]/30' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'; ?>">
				<?php esc_html_e( 'Tất cả', 'spl' ); ?>
			</a>
			<?php foreach ( $all_cats as $cat ) : ?>
				<a href="<?php echo esc_url( get_category_link( $cat ) ); ?>"
				   class="px-5 py-2.5 text-xs font-bold rounded-full transition-all whitespace-nowrap <?php echo ( $current_cat_id === $cat->term_id ) ? 'bg-[#1e73be] text-white shadow-md shadow-[#1e73be]/30' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'; ?>">
