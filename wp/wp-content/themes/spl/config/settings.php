...
add_filter( 'spl_settings_filter', '_spl_settings_filter_callback', 99 );
 ...
 * @return array
 */
function _spl_settings_filter_callback( array $arr ): array {
	...
	$settings = [
		...
		'social_follows_links' => _spl_build_social_links(),
		'contact_links'        => _spl_build_contact_links(),
		...
		// ── Form Module Config ───────────────────────────────────
		'form_config'          => _spl_build_form_config(),
	];
 ...
 */
function _spl_build_social_links(): array {
	$defs = [
	...
	foreach ( $defs as $key => [ $name, $icon, $placeholder ] ) {
		$links[ $key ] = [
			'name'        => $name,
			'icon'        => function_exists( 'hd_svg' ) ? \hd_svg( $icon ) : '',
 ...
 */
function _spl_build_contact_links(): array {
	$defs = [
	...
	foreach ( $defs as $key => [ $name, $icon, $placeholder, $target ] ) {
		$links[ $key ] = [
			'name'        => $name,
			'icon'        => function_exists( 'spl_svg' ) ? \spl_svg( $icon ) : '',
			'value'       => '',
			'placeholder' => $placeholder,
 ...
 * @return array
 */
function _spl_build_form_config(): array {
	return [
