...
final class DB {
	...
	public static function getOneWhere( string $tableName, array $where ): \WP_Error|array|null {
		...
		foreach ( $where as $col => $val ) {
			...
			if ( $val === null ) {
				$whereClauses[] = self::backtickedColumn( $col ) . ' IS NULL';
				continue;
			}
		...
		$tableFull = self::backtickedTable( $tableName );
		$sql       = 'SELECT * FROM ' . $tableFull . ' WHERE ' . implode( ' AND ', $whereClauses ) . ' LIMIT 1';
		$sql       = $values ? self::db()->prepare( $sql, ...$values ) : $sql;

		return self::db()->get_row( $sql, ARRAY_A );
	}

	/**
	 * Get one row with a raw SQL WHERE fragment.
	 *
	 * WARNING: $whereSql is interpolated into SQL. Use only trusted, hardcoded
	 * fragments with placeholders and pass all dynamic values via $params.
	 *
	 * @param string $tableName Short table name (without prefix).
	 * @param string $whereSql  Trusted SQL predicate fragment.
	 * @param array  $params    Placeholder values.
	 *
	 * @return array|null
	 */
	public static function getOneRaw( string $tableName, string $whereSql, array $params = [] ): ?array {
		$tableFull = self::backtickedTable( $tableName );
	...
	/**
	 * Backward-compatible raw getOne wrapper.
	 *
	 * @deprecated Use getOneWhere() for validated predicates or getOneRaw() for
	 *             trusted raw SQL fragments.
	 *
	 * @param string $tableName
	 ...
	 * @return array|null
	 */
	public static function getOne( string $tableName, string $whereSql, array $params = [] ): ?array {
		return self::getOneRaw( $tableName, $whereSql, $params );
	}
