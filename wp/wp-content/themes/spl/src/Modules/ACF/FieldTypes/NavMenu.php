...
class NavMenu extends \acf_field {
	...
	public function __construct() {
		$this->name     = 'nav_menu';
		$this->label    = esc_html__( 'Nav Menu', 'SPL' );
	...
	public function render_field_settings( $field ) {
		...
		acf_render_field_setting(
			$field,
			[
				'label'        => esc_html__( 'Return Value', 'SPL' ),
				'instructions' => esc_html__( 'Specify the returned value on front end', 'SPL' ),
				'type'         => 'radio',
				'name'         => 'save_format',
				'layout'       => 'horizontal',
				'choices'      => [
					'menu'   => esc_html__( 'Nav Menu HTML', 'SPL' ),
					'object' => esc_html__( 'Nav Menu Object', 'SPL' ),
					'id'     => esc_html__( 'Nav Menu ID', 'SPL' ),
				],
			]
		);
		...
		acf_render_field_setting(
			$field,
			[
				'label'        => esc_html__( 'Menu Container', 'SPL' ),
				'instructions' => esc_html__( "What to wrap the Menu's ul with (when returning HTML only)", 'SPL' ),
				'type'         => 'select',
				'name'         => 'container',
		...
		acf_render_field_setting(
			$field,
			[
				'label'   => esc_html__( 'Allow Null?', 'SPL' ),
				'type'    => 'radio',
				'name'    => 'allow_null',
				'layout'  => 'horizontal',
				'choices' => [
					1 => esc_html__( 'Yes', 'SPL' ),
					0 => esc_html__( 'No', 'SPL' ),
				],
			]
		);
	...
	public function render_field( $field ) {
		...
		if ( ! $navMenus ) {
			echo '<p class="description acf-nav-menu__empty">' . esc_html__( 'No navigation menus found. Create a menu in Appearance > Menus, then return to this field.', 'SPL' ) . '</p>';
			return;
		}
	...
	private function getNavMenus( bool $allowNull = false ): array {
		...
		if ( $allowNull ) {
			return [ '' => esc_html__( '- Select -', 'SPL' ) ] + $this->cachedMenus;
		}
