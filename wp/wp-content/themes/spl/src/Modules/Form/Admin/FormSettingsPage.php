...
final class FormSettingsPage {
	...
	public static function renderPage(): void {
		...
		$tabs = [
			'general'       => __( 'General', 'SPL' ),
			'captcha'       => __( 'CAPTCHA', 'SPL' ),
			'notifications' => __( 'Notifications', 'SPL' ),
			'spam'          => __( 'Spam & Validation', 'SPL' ),
			'cleanup'       => __( 'Cleanup', 'SPL' ),
		];
	...
	private static function renderGeneralTab( array $options ): void {
		...
		<div class="hd-form-tab-content active" id="hd-tab-general">
			<table class="form-table">
				<tr>
					<th scope="row"><label for="hd-default-emails"><?php esc_html_e( 'Default Email Recipients', 'SPL' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Fallback recipients when a form type has none configured. One email per line.', 'SPL' ); ?></p>
					</td>
				</tr>
			</table>

			<h2><?php esc_html_e( 'Email Domain Filter', 'SPL' ); ?></h2>
			<p><?php esc_html_e( 'Control which email domains are allowed or blocked. One domain per line.', 'SPL' ); ?></p>
			<table class="form-table">
				<tr>
					<th scope="row"><?php esc_html_e( 'Deny Domains', 'SPL' ); ?></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Submissions from these email domains will be rejected.', 'SPL' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Allow Domains', 'SPL' ); ?></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'If set, ONLY these domains are accepted. Leave empty to allow all (except deny list).', 'SPL' ); ?></p>
					</td>
				</tr>
			</table>
		</div>
	...
	private static function renderCaptchaTab( array $options ): void {
		...
		$providers = [
			'none'         => __( 'None (disabled)', 'SPL' ),
			'recaptcha_v2' => __( 'Google reCAPTCHA v2', 'SPL' ),
			'recaptcha_v3' => __( 'Google reCAPTCHA v3', 'SPL' ),
			'turnstile'    => __( 'Cloudflare Turnstile', 'SPL' ),
		];
		...
		<div class="hd-form-tab-content" id="hd-tab-captcha">
			<table class="form-table">
				<tr>
					<th scope="row"><label for="hd-captcha-provider"><?php esc_html_e( 'Default Provider', 'SPL' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Global default. Individual form types can override this in code config.', 'SPL' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Network Error Policy', 'SPL' ); ?></th>
					<td>
						<label>
							...
							<?php esc_html_e( 'Allow submissions when CAPTCHA provider verification has a network error.', 'SPL' ); ?>
						</label>
						<p class="description"><?php esc_html_e( 'Default is fail-closed. Enable only when provider outages are a larger business risk than spam.', 'SPL' ); ?></p>
					</td>
				</tr>
			</table>
	...
	private static function renderCaptchaProvider( string $title, string $slug, array $captcha, string $optKey, bool $hasThreshold = false ): void {
		...
		<div class="hd-captcha-provider">
			...
			<table class="form-table">
				<tr>
					<th scope="row"><?php esc_html_e( 'Site Key', 'SPL' ); ?></th>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Secret Key', 'SPL' ); ?></th>
					<td>
						<input type="password" name="<?php echo esc_attr( $optKey ); ?>[<?php echo esc_attr( $slug ); ?>_secret_key]" value="" class="regular-text code" autocomplete="new-password" placeholder="<?php esc_attr_e( 'Leave blank to keep existing.', 'SPL' ); ?>">
						<p class="description"><?php esc_html_e( 'Leave blank to keep the saved secret key.', 'SPL' ); ?></p>
					</td>
				</tr>
				<?php if ( $hasThreshold ) : ?>
				<tr>
					<th scope="row"><?php esc_html_e( 'Score Threshold', 'SPL' ); ?></th>
					<td>
						...
						<span class="description"><?php esc_html_e( '0.0 (allow all) → 1.0 (strictest)', 'SPL' ); ?></span>
					</td>
				</tr>
				<?php endif; ?>
			</table>
	...
	private static function renderNotificationsTab( array $options ): void {
		...
		<div class="hd-form-tab-content" id="hd-tab-notifications">
			<p><?php esc_html_e( 'Enable channels and configure credentials. Email is always available.', 'SPL' ); ?></p>

			<!-- Email -->
			<div class="hd-channel-block">
				<h3>
					<input type="hidden" name="<?php echo esc_attr( $optKey ); ?>[notify_email]" value="0">
					<label><input type="checkbox" name="<?php echo esc_attr( $optKey ); ?>[notify_email]" value="1" <?php checked( $emailEnabled ); ?>> <?php esc_html_e( 'Email', 'SPL' ); ?></label>
				</h3>
				<p class="description"><?php esc_html_e( 'Uses wp_mail(). Recipients are configured per form type or via Default Email Recipients in General tab.', 'SPL' ); ?></p>
			</div>

			<!-- Telegram -->
			<div class="hd-channel-block">
				<h3>
					<label><input type="checkbox" name="<?php echo esc_attr( $optKey ); ?>[notify_telegram]" value="1" <?php checked( $telegramEnabled ); ?>> <?php esc_html_e( 'Telegram', 'SPL' ); ?></label>
				</h3>
				<div class="hd-channel-fields">
					<table class="form-table">
						<tr>
							<th scope="row"><?php esc_html_e( 'Bot Token', 'SPL' ); ?></th>
							<td>
								<input type="password" name="<?php echo esc_attr( $optKey ); ?>[telegram_bot_token]" value="" class="regular-text code" autocomplete="new-password" placeholder="<?php esc_attr_e( 'Leave blank to keep existing.', 'SPL' ); ?>">
								<p class="description"><?php esc_html_e( 'Leave blank to keep the saved bot token.', 'SPL' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Chat ID', 'SPL' ); ?></th>
			...
			<div class="hd-channel-block">
				<h3>
					<label><input type="checkbox" name="<?php echo esc_attr( $optKey ); ?>[notify_viber]" value="1" <?php checked( $viberEnabled ); ?>> <?php esc_html_e( 'Viber', 'SPL' ); ?></label>
				</h3>
				<div class="hd-channel-fields">
					<table class="form-table">
						<tr>
							<th scope="row"><?php esc_html_e( 'Auth Token', 'SPL' ); ?></th>
							<td>
								<input type="password" name="<?php echo esc_attr( $optKey ); ?>[viber_auth_token]" value="" class="regular-text code" autocomplete="new-password" placeholder="<?php esc_attr_e( 'Leave blank to keep existing.', 'SPL' ); ?>">
								<p class="description"><?php esc_html_e( 'Leave blank to keep the saved auth token.', 'SPL' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Receiver', 'SPL' ); ?></th>
						...
						<tr>
							<th scope="row"><?php esc_html_e( 'Sender Name', 'SPL' ); ?></th>
						...
						<tr>
							<th scope="row"><?php esc_html_e( 'Sender Avatar URL', 'SPL' ); ?></th>
			...
			<div class="hd-channel-block">
				<h3>
					<label><input type="checkbox" name="<?php echo esc_attr( $optKey ); ?>[notify_zalo]" value="1" <?php checked( $zaloEnabled ); ?>> <?php esc_html_e( 'Zalo', 'SPL' ); ?></label>
				</h3>
				<div class="hd-channel-fields">
					<table class="form-table">
						<tr>
							<th scope="row"><?php esc_html_e( 'Bot Token', 'SPL' ); ?></th>
							<td>
								<input type="password" name="<?php echo esc_attr( $optKey ); ?>[zalo_bot_token]" value="" class="regular-text code" autocomplete="new-password" placeholder="<?php esc_attr_e( 'Leave blank to keep existing.', 'SPL' ); ?>">
								<p class="description"><?php esc_html_e( 'Leave blank to keep the saved bot token.', 'SPL' ); ?></p>
							</td>
						</tr>
						<tr>
			...
			<div class="hd-channel-block">
				<h3>
					<label><input type="checkbox" name="<?php echo esc_attr( $optKey ); ?>[digest_enabled]" value="1" <?php checked( ! empty( $digest['enabled'] ) ); ?>> <?php esc_html_e( 'Weekly Email Summary', 'SPL' ); ?></label>
				</h3>
				<div class="hd-channel-fields">
					<table class="form-table">
						<tr>
							<th scope="row"><?php esc_html_e( 'Recipients', 'SPL' ); ?></th>
							<td>
								...
								<p class="description"><?php esc_html_e( 'One email per line. Defaults to admin email if empty.', 'SPL' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Send Day', 'SPL' ); ?></th>
							<td>
								...
								<span class="description"><?php esc_html_e( 'at 8:00 AM', 'SPL' ); ?></span>
							</td>
						</tr>
					</table>
				</div>
	...
	private static function renderSpamTab( array $options ): void {
		...
		<div class="hd-form-tab-content" id="hd-tab-spam">
			<table class="form-table">
				<tr>
					<th scope="row"><?php esc_html_e( 'Spam Check', 'SPL' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="<?php echo esc_attr( $optKey ); ?>[spam_check]" value="1" <?php checked( $options['spam_check'] ?? true ); ?>>
							<?php esc_html_e( 'Enable global spam detection', 'SPL' ); ?>
						</label>
						<p class="description"><?php esc_html_e( 'Checks submissions against Akismet (if active) and WordPress Disallowed Words list (Settings → Discussion). Individual form types can override this in their code config.', 'SPL' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="hd-min-submit"><?php esc_html_e( 'Minimum Submit Time (seconds)', 'SPL' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Reject submissions faster than this. Set 0 to disable.', 'SPL' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="hd-max-render-age"><?php esc_html_e( 'Maximum Render Age (seconds)', 'SPL' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Reject stale submissions rendered before this age. Set 0 to disable.', 'SPL' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Vietnam Phone Only', 'SPL' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="<?php echo esc_attr( $optKey ); ?>[phone_vn_only]" value="1" <?php checked( ! empty( $options['phone_vn_only'] ) ); ?>>
							<?php esc_html_e( 'Only accept Vietnamese phone numbers (0xx / +84xx).', 'SPL' ); ?>
						</label>
					</td>
				</tr>
			</table>
	...
	private static function renderCleanupTab( array $options ): void {
		...
		<div class="hd-form-tab-content" id="hd-tab-cleanup">
			<p><?php esc_html_e( 'Automatic monthly cleanup of old data. Set retention period in days.', 'SPL' ); ?></p>
			<table class="form-table">
				<tr>
					<th scope="row"><label for="hd-trash-days"><?php esc_html_e( 'Trashed Entries', 'SPL' ); ?></label></th>
					<td>
						...
						<span class="description"><?php esc_html_e( 'days', 'SPL' ); ?></span>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="hd-mail-days"><?php esc_html_e( 'Mail Queue (sent/failed)', 'SPL' ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="hd-log-days"><?php esc_html_e( 'Form Logs', 'SPL' ); ?></label></th>
					<td>
