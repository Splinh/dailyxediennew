...
use SPL\Modules\Form\Security\CaptchaGuard;
use SPL\Modules\Form\Security\GeoIPResolver;
use SPL\Modules\Form\Security\HoneypotGuard;
use SPL\Modules\Form\Security\SpamChecker;
use SPL\Modules\Form\Validator\FormValidator;
...
final class FormManager {
	...
	public function processSubmission( array $input, string $ip, string $userAgent, string $referer, array $files = [] ): array|\WP_Error {
		...
		if ( is_wp_error( $entryId ) ) {
			...
			return new \WP_Error(
				'save_failed',
				__( 'An error occurred. Please try again.', 'SPL' ),
				[ 'status' => 500 ]
			);
		}
	...
	private function verifyCaptcha( string $formType, string $captchaToken, string $ip ): bool|\WP_Error {
		...
		if ( ! $guard->verify( $captchaToken, $ip ) ) {
			return new \WP_Error(
				'captcha_failed',
				__( 'CAPTCHA verification failed.', 'SPL' ),
				[ 'status' => 403 ]
			);
		}
	...
	private function storeUploadedFiles( array $files, ?callable $isUploadedFile = null, ?callable $uploadHandler = null ): array|\WP_Error {
		...
		foreach ( $files as $key => $file ) {
			...
			if ( ! is_array( $upload ) || ! empty( $upload['error'] ) ) {
				...
				$fieldErrors[ $fieldKey ] = $message ?: __( 'File upload error. Please try again.', 'SPL' );
				$diagnostics[ $fieldKey ] = [
