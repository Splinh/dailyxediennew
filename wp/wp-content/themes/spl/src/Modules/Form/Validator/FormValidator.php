 ...
 *
 * @package SPL\Modules\Form\Validator
 */

namespace SPL\Modules\Form\Validator;
...
class FormValidator {
	...
	public function validate( array $input ): array|\WP_Error {
		...
		if ( '' !== $rawEmail && ! is_email( $rawEmail ) ) {
			$fieldErrors['email'] = __( 'Invalid email address.', 'SPL' );
		}

		// 4b. Validate phone format (6–15 digits when cleaned).
		if ( '' !== $phone && ! self::isValidPhone( $phone ) ) {
			$fieldErrors['phone'] = __( 'Invalid phone number.', 'SPL' );
		}
		...
		if ( count( $rawFields ) > $maxFields ) {
			return new \WP_Error(
				'validation_failed',
				__( 'Please fix the errors below.', 'SPL' ),
				[
					'status' => 422,
					'fields' => [
						'fields' => sprintf(
							/* translators: %d: maximum allowed field count */
							__( 'Too many fields. Maximum allowed is %d.', 'SPL' ),
							$maxFields
						),
					],
				]
		...
		if ( null !== $formConfig ) {
			...
			foreach ( $required as $field ) {
				if ( empty( $fieldMap[ $field ] ) && ! isset( $fieldErrors[ $field ] ) ) {
					...
					$fieldErrors[ $field ] = sprintf( __( 'Field "%s" is required.', 'SPL' ), $field );
				}
			}
		}
	...
	public function validateFiles( array $files ): array|\WP_Error {
		...
		foreach ( $files as $key => $file ) {
			...
			if ( ! is_array( $file ) ) {
				$fieldErrors[ $fieldKey ] = __( 'Invalid upload payload.', 'SPL' );
			...
			if ( UPLOAD_ERR_OK !== (int) $file['error'] ) {
				$fieldErrors[ $fieldKey ] = __( 'File upload error. Please try again.', 'SPL' );
			...
			if ( (int) $file['size'] > $maxSize ) {
				$fieldErrors[ $fieldKey ] = __( 'File too large.', 'SPL' );
				$diagnostics[ $fieldKey ] = [
			...
			if ( $hasScriptExt || in_array( $filename, self::DANGEROUS_FILENAMES, true ) ) {
				$fieldErrors[ $fieldKey ] = __( 'This file type is not allowed for security reasons.', 'SPL' );
			...
			if ( empty( $fileType['ext'] ) || empty( $fileType['type'] ) ) {
				$fieldErrors[ $fieldKey ] = __( 'Invalid file type. Accepted: PDF, DOCX, ZIP, RAR.', 'SPL' );
		...
		if ( $fieldErrors ) {
			return new \WP_Error(
				...
				__( 'Please fix uploaded files.', 'SPL' ),
				[
					'status'        => 422,
					'fields'        => $fieldErrors,
	...
	private static function checkEmailDomain( string $email ): ?string {
		...
		if ( ! empty( $denyList ) && in_array( $domain, $denyList, true ) ) {
			return __( 'This email domain is not allowed.', 'SPL' );
		}
