...
use SPL\Modules\WooCommerce\WooCommerceModule;
...
final class FilterManager implements WooFeatureInterface, HasAPI, HasSettings {
	...
	public static function registerPostType(): void {
		register_post_type(
			...
			[
				...
				'supports'     => [ 'title' ],
				'labels'       => [
					'name'               => __( 'Filter Presets', 'SPL' ),
					'singular_name'      => __( 'Filter Preset', 'SPL' ),
					'add_new'            => __( 'Add Filter Preset', 'SPL' ),
					'add_new_item'       => __( 'Add New Filter Preset', 'SPL' ),
					'edit_item'          => __( 'Edit Filter Preset', 'SPL' ),
					'all_items'          => __( 'Filter Presets', 'SPL' ),
					'search_items'       => __( 'Search Presets', 'SPL' ),
					'not_found'          => __( 'No presets found.', 'SPL' ),
					'not_found_in_trash' => __( 'No presets found in Trash.', 'SPL' ),
				],
			]
		);
	}
	...
	public static function presetColumns( array $columns ): array {
		...
		foreach ( $columns as $key => $label ) {
			$new[ $key ] = $label;
			if ( 'title' === $key ) {
				$new['hd_layout']    = __( 'Layout', 'SPL' );
				$new['hd_filters']   = __( 'Filters', 'SPL' );
				$new['hd_shortcode'] = __( 'Shortcode', 'SPL' );
			}
		}
	...
	public static function getPresetOptions(): array {
		$options = [ 0 => __( '— None (use defaults) —', 'SPL' ) ];
		...
		foreach ( $presets as $preset ) {
			$options[ $preset->ID ] = $preset->post_title ?: __( '(no title)', 'SPL' );
		}
	...
	private static function getDefaultConfigs(): array {
		return [
			...
			[
				'id'       => 'price_1',
				'type'     => 'price_range',
				'label'    => __( 'Khoảng giá', 'SPL' ),
				'mode'     => 'custom_ranges',
				'ranges'   => [
					[
						'min'   => 0,
						'max'   => 500000,
						'label' => __( 'Dưới 500k', 'SPL' ),
					],
					[
						'min'   => 500000,
						'max'   => 1000000,
						'label' => __( '500k – 1 triệu', 'SPL' ),
					],
					[
						'min'   => 1000000,
						'max'   => 5000000,
						'label' => __( '1 – 5 triệu', 'SPL' ),
					],
					[
						'min'   => 5000000,
						'max'   => 0,
						'label' => __( 'Trên 5 triệu', 'SPL' ),
					],
				],
				'adoptive' => 'hide',
				'position' => 2,
