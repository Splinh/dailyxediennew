<?php
/**
 * OnSale Filter — boolean toggle showing only on-sale products.
 *
...
final class OnSaleFilter extends AbstractFilterType {
	...
	public function applyToQuery( array &$args, mixed $value ): void {
		...
		if ( ! empty( $args['post__in'] ) ) {
			$args['post__in'] = array_values( array_intersect( $args['post__in'], $saleIds ) );
			if ( empty( $args['post__in'] ) ) {
				$args['post__in'] = [ 0 ];
			}
		} else {
			$args['post__in'] = $saleIds;
		}
	}
