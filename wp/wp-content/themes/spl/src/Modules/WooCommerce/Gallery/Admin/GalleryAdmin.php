...
use SPL\Core\Helper;
use SPL\Modules\WooCommerce\Gallery\Frontend\GalleryDataProvider;
...
final class GalleryAdmin {
	...
	public static function renderVideoField(): void {
		...
		woocommerce_wp_text_input(
			[
				'id'          => self::VIDEO_KEY,
				'label'       => __( 'Product Video URL', 'SPL' ),
				'desc_tip'    => true,
				'description' => __( 'YouTube, Vimeo, or MP4/WEBM URL. Displayed in gallery based on Video Position setting.', 'SPL' ),
				'type'        => 'text',
		...
		woocommerce_wp_text_input(
			[
				'id'          => self::POSTER_KEY,
				'label'       => __( 'Video Poster URL', 'SPL' ),
				'desc_tip'    => true,
				'description' => __( 'Optional. Custom poster image for the video. Auto-extracted for YouTube if left empty.', 'SPL' ),
				'type'        => 'text',
				'placeholder' => 'https://...',
			]
		);
	...
	public function renderVariationField( int $loop, array $variationData, \WP_Post $variation ): void {
		...
		<div class="form-row form-row-full hd-variation-gallery" data-loop="<?php echo esc_attr( $loop ); ?>">
			<label><?php esc_html_e( 'Variation Gallery', 'SPL' ); ?></label>
			<div class="hd-variation-gallery__images">
				...
				foreach ( $galleryIds as $attachmentId ) :
					...
					<div class="hd-variation-gallery__item" data-id="<?php echo esc_attr( $attachmentId ); ?>">
						...
						<button type="button" class="hd-variation-gallery__remove" aria-label="<?php esc_attr_e( 'Remove', 'SPL' ); ?>">&times;</button>
					</div>
				<?php endforeach; ?>
			</div>
			<input type="hidden"
			...
			<button type="button" class="button hd-variation-gallery__add">
				<?php esc_html_e( 'Add Gallery Images', 'SPL' ); ?>
			</button>
		</div>
		<?php
