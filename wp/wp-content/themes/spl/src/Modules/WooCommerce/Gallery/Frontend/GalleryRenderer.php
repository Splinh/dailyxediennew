 ...
 *
 * @package SPL\Modules\WooCommerce\Gallery\Frontend
 */

namespace SPL\Modules\WooCommerce\Gallery\Frontend;
...
final class GalleryRenderer {
			...
			if ( $isProductVid ) {
				...
				<div class="hd-gallery-video <?php echo esc_attr( $aspectClass ); ?>"
					...
					<img class="hd-gallery-video__poster"
						...
						<?php endif; ?>
						loading="eager" />
					<span class="hd-gallery-video__play" aria-label="<?php esc_attr_e( 'Play video', 'SPL' ); ?>">
	...
	private static function renderVideoOverlay( string $videoUrl, string $position ): void {
		...
		<a href="<?php echo esc_url( $videoUrl ); ?>"
			...
			<?php if ( $videoTypeAttr ) : ?>
				data-video-type="<?php echo esc_attr( $videoTypeAttr ); ?>"
			<?php endif; ?>
			aria-label="<?php esc_attr_e( 'Play product video', 'SPL' ); ?>">
			<svg viewBox="0 0 24 24" fill="currentColor"><polygon points="5,3 19,12 5,21"/></svg>
		</a>
		<?php
	}
	...
	private static function renderNavArrows( string $cssPrefix ): void {
		?>
		<button type="button" class="<?php echo esc_attr( $cssPrefix ); ?> <?php echo esc_attr( $cssPrefix ); ?>--prev" aria-label="<?php esc_attr_e( 'Previous', 'SPL' ); ?>">
			<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
		</button>
		<button type="button" class="<?php echo esc_attr( $cssPrefix ); ?> <?php echo esc_attr( $cssPrefix ); ?>--next" aria-label="<?php esc_attr_e( 'Next', 'SPL' ); ?>">
