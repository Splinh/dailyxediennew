 ...
 *
 * @package SPL\Modules\WooCommerce\QuickView
 */

namespace SPL\Modules\WooCommerce\QuickView;
...
final class QuickViewManager implements WooFeatureInterface, HasAPI {
	...
	public function register(): void {
		...
		// TODO: tạm ẩn nút Quick View trên loop WooCommerce — bỏ comment để bật lại.
	...
	public static function handleAjaxAddToCart(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'wp_rest' ) ) {
			wp_send_json_error( [ 'error' => __( 'Security check failed.', 'SPL' ) ], 403 );
		}
		...
		if ( ! $productId ) {
			wp_send_json_error( [ 'error' => __( 'Invalid product.', 'SPL' ) ] );
		}

		$product = wc_get_product( $productId );
		if ( ! $product ) {
			wp_send_json_error( [ 'error' => __( 'Product not found.', 'SPL' ) ] );
		}
	...
	private static function handleGroupedAddToCart( \WC_Product $parentProduct ): void {
		...
		if ( ! $added ) {
			$errors = wc_get_notices( 'error' );
			wc_clear_notices();
			wp_send_json_error( [ 'error' => $errors ?: [ __( 'Please choose a quantity.', 'SPL' ) ] ] );
		}
	...
	public static function renderLoopButton(): void {
		...
		printf(
			...
			esc_attr__( 'Quick View', 'SPL' ),
			esc_html__( 'Quick View', 'SPL' )
		);
	}
