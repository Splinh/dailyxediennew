...
final class ProductSwatchesTab {
	...
	public function renderPanel(): void {
		...
		echo '<p class="form-field"><label>' . esc_html__( 'Archive Display', 'SPL' ) . '</label>';
		echo '<span class="description">'
			. esc_html__( 'Select which attributes to show as swatches on shop pages.', 'SPL' )
			. '</span></p>';
		...
		echo '<th>' . esc_html__( 'Attribute', 'SPL' ) . '</th>';
		echo '<th class="hd-pst-table__col-badge">' . esc_html__( 'Type', 'SPL' ) . '</th>';
		echo '<th class="hd-pst-table__col-badge">' . esc_html__( 'Terms', 'SPL' ) . '</th>';
		echo '</tr></thead><tbody>';
		...
		// ── Per-Term Overrides ──
		echo '<div class="options_group">';
		echo '<p class="form-field"><label>' . esc_html__( 'Term Overrides', 'SPL' ) . '</label>';
		echo '<span class="description">'
			. esc_html__( 'Customize swatch appearance for individual terms on this product.', 'SPL' )
			. '</span></p>';
		...
		foreach ( $termCache as $taxonomy => $terms ) {
			...
			echo '<table class="hd-pst-table"><thead><tr>';
			echo '<th class="hd-pst-table__col-term">' . esc_html__( 'Term', 'SPL' ) . '</th>';
			echo '<th class="hd-pst-table__col-type">' . esc_html__( 'Type', 'SPL' ) . '</th>';
			echo '<th class="hd-pst-table__col-value">' . esc_html__( 'Value', 'SPL' ) . '</th>';
			echo '</tr></thead><tbody>';
	...
	private function renderTermRow( string $taxonomy, \WP_Term $term, array $termOverride ): void {
		...
		echo '<span class="hd-pst-row__global">' . esc_html__( 'Global', 'SPL' ) . ': ';
		...
		echo '<option value="">' . esc_html__( '— Use global —', 'SPL' ) . '</option>';
		foreach ( [
			'color' => 'Color',
			'image' => 'Image',
			'label' => 'Label',
	...
	private function renderValueFields( string $prefix, array $termOverride, string $currentType ): void {
		...
		echo '</div>';
		echo '<button type="button" class="button button-small hd-pst-image-upload">' . esc_html__( 'Upload', 'SPL' ) . '</button>';
		printf(
			'<button type="button" class="button button-small hd-pst-image-remove"%s>%s</button>',
			! $imageId ? ' style="display:none"' : '',
			esc_html__( 'Remove', 'SPL' )
		);
		echo '</div></div>';
		...
		printf(
			...
			esc_html__( 'No additional data required.', 'SPL' )
		);
		...
		printf(
			...
			esc_html__( 'Using global settings.', 'SPL' )
		);
	}
		...
		<script>
			jQuery(function($){
				...
				P.on('click','.hd-pst-image-upload',function(e){e.preventDefault();var f=$(this).closest('.hd-pst-image-field'),
					m=wp.media({title:'<?php echo esc_js( __( 'Select Swatch Image', 'SPL' ) ); ?>',button:{text:'<?php echo esc_js( __( 'Use Image', 'SPL' ) ); ?>'},multiple:false,library:{type:'image'}});
