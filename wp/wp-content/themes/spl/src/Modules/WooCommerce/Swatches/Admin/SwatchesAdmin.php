...
use SPL\Core\Helper;
use SPL\Modules\WooCommerce\Swatches\Enum\SwatchType;
...
final class SwatchesAdmin {
	...
	private function renderFormFields( string $context, ?array $data = null ): void {
		...
		$this->renderRow(
			...
			fn() => printf(
				...
				esc_html__( 'Enable gradient (2-tone)', 'SPL' )
			),
		);
		...
		$this->renderRow(
			...
			function () use ( $data ) {
				...
				echo '<p class="description">' . esc_html__( 'Leave empty to use the term name.', 'SPL' ) . '</p>';
			},
		);
	...
	private function renderImageField( string $inputName, int $imageId ): void {
		...
		<div class="hd-swatch-image-field">
			...
			<button type="button" class="button hd-swatch-image-upload"><?php esc_html_e( 'Upload Image', 'SPL' ); ?></button>
			<button type="button" class="button hd-swatch-image-remove" <?php echo ! $imageId ? 'style="display:none"' : ''; ?>><?php esc_html_e( 'Remove', 'SPL' ); ?></button>
		</div>
		<?php
	}
	...
	private function adminScript(): string {
		$selectTitle = wp_json_encode( __( 'Select Swatch Image', 'SPL' ) ) ?: '""';
		$useImage    = wp_json_encode( __( 'Use Image', 'SPL' ) ) ?: '""';
