...
use SPL\Support\Libraries\CSS;
use SPL\Support\NavWalker\HorizontalNavWalker;
use SPL\Support\NavWalker\VerticalNavWalker;
...
trait Misc {
	...
	private static function renderNotice( string $msg, string $cssClass ): void {
		printf(
			...
			esc_attr( $cssClass ),
			esc_html( $msg ),
			esc_html__( 'Dismiss this notice.', 'SPL' )
		);
	}
	...
	public static function horizontalNav( array $args = [] ): false|string|null {
		...
		$dataAttrs = ( $dataHover ? ' data-hover="true"' : '' ) . ( $dataAutohide ? ' data-autohide="true" data-more-label="' . esc_attr__( 'More', 'SPL' ) . '"' : '' );
