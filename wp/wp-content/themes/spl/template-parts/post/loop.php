...
$itemTitle   = ! empty( $itemTitle ) ? $itemTitle : __( '(no title)', 'spl' );
$title_tag   = $args['title_tag'] ?? 'h3';
$ratio       = $args['ratio'] ?? \SPL_Helper::aspectRatioClass( get_post_type( $item_id ) );
...
$thumbnailHTML = \SPL_Helper::postImageHTML(
	$item_id,
	$thumbnail,
	[
		'alt'     => \SPL_Helper::escAttr( $itemTitle ),
		'class'   => $img_class,
		'loading' => 'lazy',
	]
);
...
if ( ! $thumbnailHTML ) {
	$thumbnailHTML = \SPL_Helper::placeholderSrc( $img_class );
}
...
<article class="item group relative flex flex-col h-full rounded-2xl border border-black/5 bg-white shadow-sm overflow-hidden transition-all duration-300 hover:border-primary/20 hover:shadow-md<?php echo $pos === 0 ? $first_class : ''; ?>">
	...
	<a href="<?php echo esc_url( $permalink ); ?>"
		...
		aria-label="<?php echo \SPL_Helper::escAttr( $itemTitle ); ?>"
	...
	<div class="flex flex-col grow p-5 gap-3">
		...
		<div class="flex flex-wrap items-center gap-2 text-xs">
			...
			<time datetime="<?php echo esc_attr( get_the_date( 'c', $item_id ) ); ?>"
				...
				<?php echo \SPL_Helper::humanizeTime( $item_id ); ?>
			</time>
		</div>

		<?php
		echo '<a class="block no-underline" href="' . esc_url( $permalink ) . '" title="' . \SPL_Helper::escAttr( $itemTitle ) . '">'
		...
		<?php echo \SPL_Query::loopExcerpt( $item_id, 'text-sm text-gray-500 leading-relaxed m-0 line-clamp-2 grow' ); ?>
		...
		<a href="<?php echo esc_url( $permalink ); ?>"
			...
			title="<?php echo esc_attr__( 'Xem chi tiết', 'spl' ); ?>">
			<?php echo __( 'Chi tiết', 'spl' ); ?>
