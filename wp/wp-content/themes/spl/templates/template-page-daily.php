...
<main id="store-content" class="max-w-7xl mx-auto px-4 py-8">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
			...
			<div class="bg-white border border-slate-100 rounded-2xl shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] overflow-hidden">
				...
				<div class="p-4 md:p-5 space-y-3 border-b border-slate-100 bg-slate-50/50">
					<div class="relative">
						<input type="text" id="locator-search" placeholder="<?php esc_attr_e( 'Nhập từ khoá tìm kiếm theo tên', 'spl' ); ?>" class="w-full pl-4 pr-10 py-2.5 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-[#1e73be] transition-all" />
						<button class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#1e73be]" aria-label="<?php esc_attr_e( 'Tìm kiếm', 'spl' ); ?>">
					...
					<div class="grid grid-cols-2 gap-3">
						<select id="locator-province" class="px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-[#1e73be] transition-all cursor-pointer">
						...
						<select id="locator-type" class="px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-[#1e73be] transition-all cursor-pointer">
			...
			<div class="bg-white border border-slate-100 rounded-2xl shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] overflow-hidden relative">
				...
				<div id="map-info-overlay" class="hidden absolute top-4 left-4 right-4 md:right-auto md:w-[380px] bg-white rounded-2xl shadow-[0_10px_30px_-5px_rgba(0,0,0,0.15)] border border-slate-100 p-4 z-10 transition-all duration-300">
					<!-- Close button -->
					<button onclick="window.closeMapOverlay()" class="absolute top-2.5 right-2.5 w-6 h-6 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center text-xs transition-colors" aria-label="<?php esc_attr_e( 'Đóng', 'spl' ); ?>">
	...
	<section class="mb-6">
		<div class="relative">
			...
			<div id="province-scroll" class="flex gap-2 overflow-x-auto px-10 py-1.5 scroll-smooth" style="-ms-overflow-style:none;scrollbar-width:none;">
				...
				$first = true;
				foreach ( $provinces as $prov ) :
					$active_cls = $first ? 'bg-emerald-500 text-white shadow-md' : 'bg-slate-100 text-slate-700 hover:bg-slate-200';
					?>
					<button onclick="filterProvince('<?php echo esc_js( $prov->name ); ?>', this)"
							class="prov-btn px-4 py-2 text-xs font-bold rounded-full transition-all whitespace-nowrap <?php echo $active_cls; ?>">
		...
		<div class="flex items-center gap-6 border-b border-slate-200 pb-3">
			...
			foreach ( $store_types as $st ) :
				...
				<button onclick="switchStoreTab('<?php echo esc_js( $st->slug ); ?>', this)"
						class="store-tab-btn flex items-center gap-2 text-sm font-bold pb-2 transition-all <?php echo $tab_cls; ?>">
					<?php if ( $first_type ) : ?>
						<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
					<?php else : ?>
	...
	<!-- ===== CTA: BECOME A DEALER ===== -->
	<section class="mb-16">
		<div class="bg-gradient-to-br from-[#002647] to-[#1e73be] rounded-2xl p-8 md:p-12 text-white relative overflow-hidden">
			<div class="absolute top-0 right-0 w-40 h-40 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2"></div>
			<div class="absolute bottom-0 left-0 w-32 h-32 bg-emerald-500/10 rounded-full translate-y-1/2 -translate-x-1/2"></div>
			<div class="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
				<div class="max-w-xl">
					<h2 class="text-xl md:text-2xl font-black mb-2"><?php esc_html_e( 'Trở thành đối tác của dailyxedien.vn?', 'spl' ); ?></h2>
					<p class="text-sm text-white/75 leading-relaxed"><?php esc_html_e( 'Mở đại lý xe điện với thương hiệu uy tín, nhận hỗ trợ marketing, đào tạo nhân viên và nguồn hàng chính hãng từ 50+ thương hiệu.', 'spl' ); ?></p>
				</div>
				<div class="flex items-center gap-3 shrink-0">
					...
					?>
					<a href="<?php echo esc_url( $hop_tac_url ); ?>" class="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold text-sm transition-colors flex items-center gap-2 shadow-lg shadow-emerald-500/30">
						<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M7 11v8a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1v-7a1 1 0 0 1 1-1h3m0 0V7a4 4 0 0 1 4-4h0a4 4 0 0 1 4 4v4h3a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1h-2a1 1 0 0 1-1-1v-8"/></svg>
						<?php esc_html_e( 'Đăng ký ngay', 'spl' ); ?>
					</a>
					<a href="<?php echo esc_url( $hotline_url ); ?>" class="bg-white/15 hover:bg-white/25 text-white px-5 py-3 rounded-xl font-bold text-sm transition-colors flex items-center gap-2">
						<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72"/></svg>
						<?php esc_html_e( 'Gọi tư vấn', 'spl' ); ?>
					</a>
				</div>
			</div>
		</div>
...
(function() {
	...
	window.filterProvince = function(name, btn) {
		...
		document.querySelectorAll('.prov-btn').forEach(function(b) {
			b.className = b.className.replace(/bg-emerald-500 text-white shadow-md/g, 'bg-slate-100 text-slate-700 hover:bg-slate-200');
		});
		if (btn) {
			btn.className = btn.className.replace(/bg-slate-100 text-slate-700 hover:bg-slate-200/g, 'bg-emerald-500 text-white shadow-md');
		}
		renderCards();
	};
	...
	function renderCards() {
		...
		c.innerHTML = f.map(function(s) {
			...
			var phonesHtml = phones.map(function(phone, idx) {
				var iconClass = idx === 0 ? 'fa-solid fa-phone' : 'fa-solid fa-mobile-screen-button';
				var colorClass = idx === 0 ? 'text-blue-500' : 'text-amber-500';
				var cleanPhone = phone.replace(/\s+/g, '');
				return '<a href="tel:' + cleanPhone + '" class="flex items-center gap-1.5 hover:underline text-slate-700">' +
					'<i class="' + iconClass + ' ' + colorClass + '"></i> ' + phone + '</a>';
			}).join('');
				...
				'<div>' +
					...
					'<div class="p-5 space-y-4">' +
						'<div>' +
							...
							'</div>' +
						'</div>' +
						(phonesHtml ? '<div class="flex flex-wrap gap-x-4 gap-y-1.5 text-xs font-semibold py-2 border-y border-slate-50">' + phonesHtml + '</div>' : '') +
					'</div>' +
				'</div>' +
				'<!-- Actions & Address -->' +
		...
		if (s) {
			...
			if (overlay && content) {
				...
				var phonesHtml = phones.map(function(p) { 
					return '<p class="flex items-center gap-1.5"><i class="fa-solid fa-phone text-blue-500 shrink-0"></i> <strong class="text-slate-700">' + p + '</strong></p>'; 
				}).join('');
				var mapUrl = (s.la && s.lo) ? 'https://www.google.com/maps/dir//' + s.la + ',' + s.lo + '/' : 'https://maps.google.com/?q=' + encodeURIComponent(s.t + ', ' + s.a);

				content.innerHTML = '<div class="flex gap-4 items-start">' +
					'<div class="w-24 h-18 rounded-lg overflow-hidden bg-slate-100 shrink-0"><img src="' + img + '" alt="' + s.t + '" class="w-full h-full object-cover"></div>' +
					'<div class="flex-1 min-w-0 text-left">' +
						'<h3 class="font-black text-slate-800 text-xs uppercase leading-snug break-words">' + s.t + '</h3>' +
						'<span class="inline-block mt-1 text-[9px] font-bold px-1.5 py-0.5 rounded border ' + badge + '">' + s.tn + '</span>' +
						'<div class="mt-2 space-y-1 text-[11px] text-slate-500">' +
							'<p class="flex items-start gap-1"><i class="fa-solid fa-location-dot text-slate-400 mt-0.5 shrink-0"></i> ' + s.a + '</p>' +
							phonesHtml +
						'</div>' +
					'</div>' +
				'</div>' +
				'<div class="flex items-center justify-start gap-2 mt-3.5 border-t border-slate-50 pt-3">' +
					'<a href="' + mapUrl + '" target="_blank" class="border border-emerald-600 text-emerald-600 hover:bg-emerald-600 hover:text-white rounded-full px-3.5 py-1.5 text-[10px] font-black flex items-center gap-1 transition-all">' +
						'<i class="fa-solid fa-paper-plane text-[9px]"></i> Chỉ đường' +
					'</a>' +
					'<a href="' + s.u + '" class="btn-detail-brand border rounded-full px-3.5 py-1.5 text-[10px] font-black flex items-center gap-1">' +
						'<i class="fa-solid fa-circle-info text-[9px]"></i> Xem chi tiết' +
					'</a>' +
				'</div>';
		...
		c.innerHTML = f.map(function(s) {
			...
			return '<div onclick="window.selectLocatorStore(' + s.id + ')" class="p-4 hover:bg-slate-50/50 cursor-pointer transition-all ' + activeCls + '">' +
				'<div class="flex gap-4">' +
					'<div class="w-28 h-20 rounded-lg overflow-hidden bg-slate-100 shrink-0"><img src="' + img + '" alt="' + s.t + '" class="w-full h-full object-cover" loading="lazy"></div>' +
					'<div class="flex-1 min-w-0">' +
						'<h3 class="font-black text-slate-800 text-sm uppercase leading-snug line-clamp-1 group-hover:text-[#1e73be] transition-colors">' + s.t + '</h3>' +
						'<span class="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded border ' + badge + '">' + s.tn + '</span>' +
						'<div class="mt-2 space-y-1 text-xs text-slate-500"><p class="flex items-start gap-1.5 line-clamp-2"><i class="fa-solid fa-location-dot text-slate-400 mt-0.5 shrink-0"></i> ' + s.a + '</p>' + phonesHtml + '</div>' +
						'<div class="flex items-center gap-2 mt-2.5">' +
							'<a href="' + mapUrl + '" target="_blank" onclick="event.stopPropagation();" class="border border-emerald-600 text-emerald-600 hover:bg-emerald-600 hover:text-white rounded-full px-3.5 py-1.5 text-[10px] font-black flex items-center gap-1 transition-all"><i class="fa-solid fa-paper-plane text-[9px]"></i> Chỉ đường</a>' +
							'<a href="' + s.u + '" onclick="event.stopPropagation();" class="btn-detail-brand border rounded-full px-3.5 py-1.5 text-[10px] font-black flex items-center gap-1"><i class="fa-solid fa-circle-info text-[9px]"></i> Xem chi tiết</a>' +
						'</div>' +
					'</div>' +
				'</div>' +
			'</div>';
